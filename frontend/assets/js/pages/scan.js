    ...
    pollingInterval = setInterval(async () => {
        try {
            ...
            if (data.status === "In Queue") {
                ...
                // Add the native WhatsApp button for the patient
                if (waitStatusEl) {
                    waitStatusEl.innerHTML = `
                        ...
                        <button onclick="window.location.href='whatsapp://'" style="background: #25D366; color: white; padding: 14px 24px; border-radius: 12px; border: none; font-size: 16px; font-weight: bold; cursor: pointer; width: 100%; box-shadow: 0 4px 12px rgba(37, 211, 102, 0.3); display: flex; align-items: center; justify-content: center; gap: 10px;">
                ...
                // Crucial: Clear the local token so the next time they scan, they get a fresh session.
                wipeVolatileMemory();
